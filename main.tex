...
\begin{frame}[fragile]{ScaROOT-Reader: iterating over ROOT TTrees}

\vfill
\fbox{\url{http://github.com/diana-hep/rootconverter}}

\vfill
Two projects: pure-C++ {\tt root2avro} converter and {\tt ScaROOT-Reader}.
...
// The brackets (equivalent of C++ template) invokes the compile-time macro
// to generate custom filling code for TwoMuon.
val iterator = RootTreeIterator[TwoMuon](
  List("root://cmseos.fnal.gov/TrackResonanceNtuple.root"), "twoMuon")

// Do anything with this data stream in the JVM.
while (iterator.hasNext) {
  val datum = iterator.next()
  println(s"mass: ${datum.mass_mumu} pt: ${datum.pt}")
}
\end{lstlisting}

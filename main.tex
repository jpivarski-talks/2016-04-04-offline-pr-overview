...
\textcolor{darkblue}{aggregate} & table of $A$, initial value $B$ & $f: (A, B) \to B$ and also $f: (B, B) \to B$ & single value $B$ & accumulate a counter with a different data type from the input \\
\textcolor{darkblue}{aggregate by key} & table of $\langle K,A \rangle$, initial value $B$ & same as above & pairs $\langle K,B \rangle$ & aggregate independently for each key \\

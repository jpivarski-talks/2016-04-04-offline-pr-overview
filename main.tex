...
\begin{description}
\item[CSV, JSON, XML:] text-based for human readability; more common than you'd think.
\item[Avro, Thrift, and Protocol buffers:] generic, row-based binary data, like JSON with a schema.
\item[Parquet, ORC, RCFile:] generic, columnar binary data, used for SQL-style analyses.
\item[Java serialization, Kryo, Hadoop Writables:] intermediate serialization for data in a distributed stream.
\item[Python pickle files:] persist Python objects.
\item[RData, RDS:] persist R objects.
\end{description}
\end{frame}

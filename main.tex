...
One thing most of these toolkits have in common is that they are written for the Java Virtual Machine.
...
\textcolor{darkblue}{C/C++} & \textcolor{darkblue}{JVM (Java, Scala, etc.)} & \textcolor{darkblue}{Python and R} \\
\vspace{-0.2 cm} Fast, but hard to debug. Mixes data analysis with low-level concerns. & \vspace{-0.2 cm} Type-aware bytecode is moderately fast, but no access to hardware. & \vspace{-0.2 cm} Not supposed to be fast: offload heavy work on compiled external packages.
\end{tabular}
\end{frame}

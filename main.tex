...
\item {\tt Binned(f, Counter)} is a standard 1-d histogram with a fill rule $f$.
\item {\tt Binned(f, MeanRms)} is a ``profile plot.''
\item {\tt Binned(f, Binned(g, Counter))} is a standard 2-d histogram with a fill rule $f$ for one dimension and $g$ for the other.
...
But currently, the aggregation is built in complex, imperative analysis scripts. Imagine if they could be built from a few fundamental building blocks?

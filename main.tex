...
There are FIVE stream-processors maintained by Apache:
...
Again, many ways to do the same thing.

\vfill
\hspace{-0.4 cm}\begin{minipage}{\linewidth}
\renewcommand{\arraystretch}{1.5}
\begin{tabular}{p{0.4\linewidth} p{0.6\linewidth}}
JSON, XML, CSV\ldots & Text-based for human readability; more common than you'd think. \\
\raggedright Avro, Thrift, Protocol buffers\ldots & Generic, row-based binary data, like JSON with a schema. \\
Parquet, ORC, RCFile\ldots & Generic, columnar binary data, used for SQL-style analyses. \\
\raggedright Java serialization, Kryo, Hadoop Writables\ldots& Intermediate serialization for data in a distributed stream. \\
Python pickle files & Persist Python objects between sessions. \\
RData, RDS & Persist R objects between sessions. \\
\end{tabular}
\end{minipage}
\vfill
\end{frame}
...
\vspace{2 cm}
But perhaps we can use what's out there and let the wider community maintain it.

\vspace{0.2 cm}
For every pair of software products, {\it somebody's} written an adapter to connect them.
\end{minipage}
\end{center}
\end{frame}

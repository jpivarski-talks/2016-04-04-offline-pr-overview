...
Monojet analysis using Spark:
\begin{itemize}
\item Oliver Gutsche
\item Matteo Cremonesi
\item Cristina Su\'arez (produced plot at right)
\end{itemize}

Part of a larger effort to use Spark in HEP:
\begin{itemize}
...
\begin{lstlisting}[language=scala]
val rdd = fileListRDD.flatMap(x => RootTreeIterator[Tree](
  x.map(_.url), treeLocation, libs, myclasses))
\end{lstlisting}
\end{frame}

...
One thing most of these toolkits have in common is that they are written for the Java Virtual Machine (Java, Scala, Clojure, etc.).



\renewcommand{\arraystretch}{1.0} \begin{tabular}{>{\raggedright}p{0.3\linewidth} >{\raggedright}p{0.3\linewidth} >{\raggedright\arraybackslash}p{0.3\linewidth}}
...
Fastest, raw machine access, \mbox{static bytecode,\hspace{-1 cm}} manual memory management. & Medium speed, dynamic optimizer. Garbage collector is fast but pauses. & Slowest, though performance-critical code is external. \\
Used by physicists, high-frequency trading, cybersecurity. & Used for large-scale business analytics; suited to networking. & Used by statisticians and data scientists for laptop-analyses.
\end{tabular}

...
\begin{frame}[fragile]{}
ScaROOT-Reader is being used in a CMS monojet analysis:
\begin{columns}
...
Part of a larger Big Data in HEP effort:
\begin{itemize}
...
\vfill
Producing a distributed TTree iterator in Spark:

\begin{lstlisting}[language=scala]
val rdd = xrootd_url_rdd.flatMap(x => RootTreeIterator[Tree](
  x.map(_.xrootd_url), tree_location_in_file, libs, my_class_definitions))
\end{lstlisting}
\end{frame}

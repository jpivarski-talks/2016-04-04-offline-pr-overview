...
\renewcommand{\arraystretch}{1.0} \begin{tabular}{>{\raggedright}p{0.27\linewidth} >{\raggedright}p{0.35\linewidth} >{\raggedright\arraybackslash}p{0.27\linewidth}}
\textcolor{darkblue}{C/C++} & \textcolor{darkblue}{JVM (Java, Scala\ldots)} & \textcolor{darkblue}{Python and R} \\
\vspace{-0.2 cm} Fast, but hard to debug. Mixes data analysis with low-level concerns. & \vspace{-0.2 cm} Type-aware bytecode is moderately fast. Great debugging support. & \vspace{-0.2 cm} Not supposed to be fast: offload heavy work on compiled external packages. Highly dynamic.
\end{tabular}
\end{frame}
